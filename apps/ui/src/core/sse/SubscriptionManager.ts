...
import type { Subscription } from "./SSEContext";
	...
	return {
		connect() {
			...
			state.source.onmessage = (e: MessageEvent) => {
				const ev = JSON.parse(e.data) as {
					event?: string;
					data?:
						| { uuid?: string; name?: string; state?: unknown }
						| unknown[]
						| unknown;
				};
				if (ev.event === "connect") {
				...
				if (ev.event) {
					...
					if (
						...
						if (dataObj.name) {
							state.evSubs
								.get(`${ev.event}.${dataObj.name}`)
								?.forEach((sub) => {
									sub.callback(eventData);
								});
						}
						if (dataObj.state !== undefined) {
							state.evSubs.get(ev.event)?.forEach((sub) => {
								sub.callback(dataObj.state);
							});
						} else {
							state.evSubs.get(ev.event)?.forEach((sub) => {
								sub.callback(eventData);
						...
						eventData.forEach((d: unknown) => {
							...
							if (dObj && typeof dObj === "object" && dObj.name) {
								state.evSubs.get(`${ev.event}.${dObj.name}`)?.forEach((sub) => {
									sub.callback(dObj.state);
								});
							} else {
								state.evSubs.get(ev.event)?.forEach((sub) => {
									sub.callback(d);
			...
			state.source.onerror = (e: Event) => {
				...
				state.onError?.(new Error("Error connecting to server"));
			};
		},
		onConnect(fn: () => void) {
			state.onConnect = fn;
		...
		async subscribe(sub: Subscription) {
			if (state.evSubs.has(sub.event)) {
				...
				state.providers.set(
					sub.provider,
					state.providers.get(sub.provider) + 1,
				);
				...
				if (state.providers.has(sub.provider)) {
					...
					const response = await axios.get(
						`${url}/${sub.provider}/subscribe/${state.sessionId}`,
					);
		...
		unsubscribe(subId: string) {
			state.subs.get(subId)?.forEach((sub) => {
				state.evSubs
					.get(sub.event)
					?.splice(state.evSubs.get(sub.event)?.indexOf(sub), 1);
				...
				if (state.providers.get(sub.provider) <= 0) {
					state.providers.delete(sub.provider);
					axios
						.get(`${url}/${sub.provider}/unsubscribe/${state.sessionId}`)
