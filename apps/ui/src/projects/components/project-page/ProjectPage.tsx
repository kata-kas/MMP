			...
			<Header
				...
				imagePath={
					settings?.localBackend
						? `${settings.localBackend}/projects/${project?.uuid}/assets/${project?.default_image_id}/file`
						: undefined
				}
				onTagClick={(t) =>
					navigate(
						`/projects/list?filter=${JSON.stringify({ tags: [t.value] })}`,
					)
				}
			/>
			{error && !loading && (
