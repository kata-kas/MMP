...
export function UploadPreview({
	...
	return (
		<div className="mt-4 grid grid-cols-3 gap-4">
			{files
				.sort((f1) => (isImage(f1) ? -1 : 0))
				.map((f, i) => (
					<Card key={`file-${f.name}-${i}`} className="p-0 overflow-hidden">
						<div className="flex items-center gap-0">
							{isImage(f) ? (
								...
								<AspectRatio
									ratio={1}
									className="flex-shrink-0 w-[100px] flex items-center justify-center bg-muted"
								>
