				...
				{Array.from({ length: 50 }).map((_, i) => (
					<div key={i} className="space-y-2">
						...
						<p className="text-sm text-muted-foreground">
							This is some content for section {i + 1}. It demonstrates how the
							scroll area handles long content that extends beyond the visible
							area.
						</p>
					</div>
				))}
		...
		<ScrollArea className="w-[350px] whitespace-nowrap rounded-md border">
			<div className="flex w-max space-x-4 p-4">
				{Array.from({ length: 20 }).map((_, i) => (
					<div
						key={i}
						className="flex h-32 w-32 shrink-0 items-center justify-center rounded-md border"
					>
						Item {i + 1}
					</div>
				))}
		...
		<ScrollArea className="h-[250px] w-[300px] rounded-md border">
			<div className="p-4">
				...
				{Array.from({ length: 30 }).map((_, i) => (
					<div key={i} className="text-sm">
						<div className="mb-2 rounded-md border p-2">Tag {i + 1}</div>
					</div>
				))}
			</div>
		</ScrollArea>
