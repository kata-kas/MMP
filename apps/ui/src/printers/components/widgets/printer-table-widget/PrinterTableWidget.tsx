...
import type { Job, Printer, Thermal } from "@/printers/entities/Printer";
...
export function PrinterTableWidget(w: Widget) {
	...
	const [heaterBed, setHeaterBed] = useCumulativeEvent<Thermal>({
		temperature: 0,
	});
	const [job, setJob] = useCumulativeEvent<Job>({
		progress: 0,
		fileName: "",
		message: "",
	});
		...
		<Card>
			...
			<div className="border-t p-3">
				<div className="flex items-center justify-between">
					...
					<p className="font-medium">
						{(extruder.temperature ?? 0).toFixed(1)}°C
					</p>
				</div>
			</div>
			<Separator />
			<div className="border-t p-3">
				<div className="flex items-center justify-between">
					...
					<p className="font-medium">
						{(heaterBed?.temperature ?? 0).toFixed(1)}°C
					</p>
				</div>
			</div>
		</Card>
