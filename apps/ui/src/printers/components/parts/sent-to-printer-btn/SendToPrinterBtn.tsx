...
export function SendToPrinterBtn({ id }: SentToPrinterBtnProps) {
	...
	const sendToPrinterMutation = useApiMutation<
		void,
		{ printerUuid: string; fileId: string }
	>({
	...
	function sentToPrinter(p: Printer) {
		sendToPrinterMutation
			.mutate({ printerUuid: p.uuid, fileId: id })
			.then(() => {
		...
		<DropdownMenu>
			<DropdownMenuTrigger asChild>
				<Button
					variant="ghost"
					size="icon"
					disabled={loading || sendToPrinterMutation.loading}
				>
