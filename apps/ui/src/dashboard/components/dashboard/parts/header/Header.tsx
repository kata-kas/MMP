...
import type {
	DashboardItem,
	WidgetType,
} from "@/dashboard/entities/WidgetType";
...
export function Header({
	...
	const addWidget = useCallback(() => {
		...
		reset();
		setOpened(false);
	}, [config, selectedType, addItem, reset]);
		...
		<>
			<div className="flex justify-end gap-2">
				...
				<Button variant="ghost" size="icon" onClick={toggleEdit}>
					{edit ? (
						<IconSettingsOff className="h-4 w-4" />
					) : (
				...
				<SheetContent side="right">
					...
					{selectedType && (
						<div className="mt-4 space-y-4">
							{React.cloneElement(selectedType.configElement, {
								config: config,
								onChange: setConfig,
							})}
