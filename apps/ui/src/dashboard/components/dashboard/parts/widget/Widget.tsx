...
export function Widget({ model, edit, onDelete }: WidgetProps) {
    const { widgetTypes } = useContext(dashboardContext) as { widgetTypes: Map<string, { name?: string; element: React.ReactElement; configElement: React.ReactElement }> }
    const widgetType = widgetTypes.get(model.type)
    const [config, setConfig] = useState(model.config)
    const w = widgetType ? React.cloneElement(widgetType.element, { config, onChange: () => { } }) : null
    const c = widgetType ? React.cloneElement(widgetType.configElement, { config, onChange: setConfig }) : null
