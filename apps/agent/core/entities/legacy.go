...
// Deprecated: ProjectAsset is deprecated. Use Asset instead.
// Kept for migration and legacy code compatibility.
...
func assetSha(projectUuid string, assetName string, fullFilePath string) (string, error) {
	...
	return fmt.Sprintf("%x", sha1.Sum(fmt.Appendf(nil, "%s%s%s", projectUuid, assetName, fSha512))), nil
}
